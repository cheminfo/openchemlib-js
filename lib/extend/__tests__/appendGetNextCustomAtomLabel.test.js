...
test('edge cases', () => {
  const molecule = Molecule.fromSmiles('CCO');
  molecule.setAtomCustomLabel(0, `100a'`);
  molecule.setAtomCustomLabel(1, 'z');
  molecule.setAtomCustomLabel(2, `a100''`);
  expect(molecule.getNextCustomAtomLabel(`100a'`)).toBe(`101a'`);
  expect(molecule.getNextCustomAtomLabel('z')).toBe('1');
  expect(molecule.getNextCustomAtomLabel(`a100''`)).toBe(`a101''`);
});
