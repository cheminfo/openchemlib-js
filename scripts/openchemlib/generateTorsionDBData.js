...
const files = ['torsionID', 'torsionAngle', 'torsionRange', 'torsionFrequency', 'torsionBins', 'bondLengthData'];
...
function generateTorsionDBData({ config }) {
  ...
  for (const file of files) {
    const contents = fs.readFileSync(path.join(config.openchemlib, folder, `${file}.txt`), 'utf8');
